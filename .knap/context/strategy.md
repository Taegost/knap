...
**OKF-aligned.** Knap's format aligns with Google's Open Knowledge Format spec. Same core structure. Key differences: separate AI/human directories, category-specific fields.
...
**Single-use scripts.** Favor scripts scoped to a single task over large, monolithic scripts. A link validator validates links. A linter orchestrates checks. Each script does one thing well; composed scripts call smaller ones rather than duplicating logic.
